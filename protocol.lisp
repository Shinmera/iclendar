...
(defmethod initialize-instance :around ((slot direct-parameter-slot) &rest args)
  (let ((name (getf args :name)))
    (unless (getf args :initargs)
      (setf (getf args :initargs) (list (intern (string name) :keyword))))
    (unless (or (getf args :readers)
                (getf args :writers))
      (setf (getf args :readers) (list name))
      (setf (getf args :writers) (list `(setf ,name))))
...
(defmethod initialize-instance :around ((slot direct-property-slot) &rest args)
